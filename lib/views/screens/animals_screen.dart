...
class _AnimalsScreenState extends State<AnimalsScreen> {
  ...
  final List<LearnItem> _animals = const [
    LearnItem(id: 'cat', big: '😺', emoji: '😺', imageAsset: 'src/Images/Cat.png', audioNumber: 50),
    LearnItem(id: 'dog', big: '🐶', emoji: '🐶', imageAsset: 'src/Images/Dog.png', audioNumber: 51),
    LearnItem(id: 'cow', big: '🐮', emoji: '🐮', imageAsset: 'src/Images/Cow.png', audioNumber: 52),
    LearnItem(id: 'sheep', big: '🐑', emoji: '🐑', imageAsset: 'src/Images/Sheep.png', audioNumber: 53),
    LearnItem(id: 'horse', big: '🐴', emoji: '🐴', imageAsset: 'src/Images/Horse.png', audioNumber: 54),
    LearnItem(id: 'lion', big: '🦁', emoji: '🦁', imageAsset: 'src/Images/Lion.png', audioNumber: 55),
    LearnItem(id: 'elephant', big: '🐘', emoji: '🐘', imageAsset: 'src/Images/Elephant.png', audioNumber: 56),
    LearnItem(id: 'rabbit', big: '🐰', emoji: '🐰', imageAsset: 'src/Images/Rabbit.png', audioNumber: 57),
    LearnItem(id: 'duck', big: '🦆', emoji: '🦆', imageAsset: 'src/Images/Duck.png', audioNumber: 58),
    LearnItem(id: 'fish', big: '🐟', emoji: '🐟', imageAsset: 'src/Images/Fish.png', audioNumber: 59),
  ];
  ...
  void _makeRound() {
    ...
    final set = _shuffle(_animals).take(k).toList(growable: false);
  ...
  Future<void> _speakPrompt() async {
    final it = _animals.where((x) => x.id == _promptId).cast<LearnItem?>().firstOrNull;
    if (it == null) return;
    ...
    return ListView(
      children: [
        _HeroCard(
          ...
          gradient: const [Color(0xFF2EC4B6), Color(0xFF00BBF9), Color(0xFF9B5DE5)],
          onPressed: () async {
            setState(() {
              _playingAll = true;
              _activeId = null;
            });
            final list = _animals.map((x) => _audioPath(x.audioNumber)).toList(growable: false);
            await context.read<AudioProvider>().playSequence(
              list,
              onStep: (i) => setState(() => _activeId = _animals[i].id),
            );
            if (!mounted) return;
            setState(() {
              _playingAll = false;
        ...
        LayoutBuilder(
          builder: (context, c) {
            final cols = c.maxWidth >= 860 ? 5 : (c.maxWidth >= 520 ? 3 : 2);
          ...
          child: Column(
            children: [
              Row(
                ...
                children: [
                  const Text('🎮 Hayvon o\'yini', style: TextStyle(fontWeight: FontWeight.w900)),
                      ...
                      return InkWell(
                        ...
                        child: Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Image.asset(it.imageAsset!, width: 92, height: 92, fit: BoxFit.contain),
                              const SizedBox(height: 8),
                              Text(it.emoji ?? '', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 28)),
                            ],
                          ),
                        ),
                      );
